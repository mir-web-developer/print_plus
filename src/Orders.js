...
const Orders = () => {
  ...
  const [isLoading, setIsLoading] = useState(false);
  const [searchDate, setSearchDate] = useState(null);
  const ordersPerPage = 6; // Количество заказов на каждой странице
  const db = getFirestore(firebaseApp);
  ...
  const handleSavePhotos = async () => {
    if (selectedOrder && selectedPhotos.length > 0) {
      try {
        ...
        const storage = getStorage(firebaseApp);
        const storageRef = ref(storage, `${selectedOrder.id}/`);
    ...
    return filteredOrders.slice(startIndex, endIndex).map((order) => (
      <Grid item xs={12} sm={6} md={4} key={order.id}>
        <Paper
          ...
          style={{
            padding: "16px",
            backgroundColor: shouldPickUpToday(order)
              ? "RGBa(255,87,51,0.5)"
